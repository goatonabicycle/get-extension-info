	...
	try {
		...
		const webDate = await page.evaluate(() => {
			...
			if (lastUpdatedEl) {
				...
				const dateMatch = lastUpdatedEl.textContent.match(
				...
				const dateMatch2 = lastUpdatedEl.textContent.match(
...
async function fetchFirefoxExtensionInfo(slug) {
	...
	try {
		...
		if (webDate) {
			const expandedDate = webDate.replace(
			...
			extensionData.lastUpdated =
				extractDateFromText(expandedDate) || extensionData.lastUpdated;
		}
