...
async function fetchGitLabReleases() {
	...
	try {
		...
		const url =
			"https://gitlab.com/eyeo/browser-extensions-and-premium/extensions/extensions/-/releases";
		...
		await page.waitForSelector(
			'h2, .release-item, [data-testid="release-block"]',
			{ timeout: 30000 },
		);
		...
		const releases = await page.evaluate(() => {
			...
			const releaseHeaders = Array.from(
				document.querySelectorAll(
					'h2, h3, .release-title, [data-testid*="release"]',
				),
			).concat(
				Array.from(document.querySelectorAll("*")).filter((el) => {
					const text = el.textContent || "";
					return (
						text.match(/^(Adblock Plus|AdBlock)\s+\d+\.\d+\.\d+(\.\d+)?$/) ||
			...
			for (const header of releaseHeaders) {
				...
				const adblockPlusMatch = headerText.match(
					/^Adblock Plus\s+(\d+\.\d+\.\d+(\.\d+)?)$/,
				);
				const adblockMatch = headerText.match(
					/^AdBlock\s+(\d+\.\d+\.\d+(\.\d+)?)$/,
				);
				...
				if (adblockPlusMatch || adblockMatch) {
					...
					for (let i = 0; i < 5 && parent; i++) {
						...
						const dateMatch = dateText.match(
							/(\d{4}-\d{2}-\d{2})|((Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2},?\s+\d{4})/,
						);
						if (dateMatch) {
					...
					const timeElements =
						header.parentElement?.querySelectorAll("time") || [];
					for (const timeEl of timeElements) {
						if (
							timeEl.dateTime ||
							timeEl.textContent.match(/\d{4}-\d{2}-\d{2}/)
						) {
							releaseDate =
								timeEl.dateTime ||
								timeEl.textContent.match(/\d{4}-\d{2}-\d{2}/)[0];
						}
					}

					const existing = releases.find(
						(r) => r.extension === extension && r.version === version,
					);
					if (!existing) {
						releases.push({
							extension,
		...
		releases.forEach((release, i) => {
			console.log(
				`${i + 1}. ${release.extension} v${release.version} - ${release.releaseDate || "No date found"}`,
			);
		...
		if (releases.length === 0) {
			...
			const debugInfo = await page.evaluate(() => {
				const allHeaders = Array.from(
					...
					.slice(0, 10);
				const releaseWords = Array.from(document.querySelectorAll("*"))
					.filter((el) => el.textContent?.toLowerCase().includes("adblock"))
...
function getLatestGitLabReleases(releases) {
	...
	const adblockReleases = releases
		.filter((r) => r.extension === "adblock")
		.sort((a, b) => new Date(b.releaseDate) - new Date(a.releaseDate));
	const adblockPlusReleases = releases
		.filter((r) => r.extension === "adblockplus")
		.sort((a, b) => new Date(b.releaseDate) - new Date(a.releaseDate));
