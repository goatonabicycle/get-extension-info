...
describe("Parsing Functions", () => {
	test("Version parser extracts semantic version numbers", () => {
	...
	test("Date parser extracts update dates", () => {
		...
		expect(getDateFromMatch("Published: January 15, 2024")).toBe(
			"January 15, 2024",
		);
		expect(getDateFromMatch("Released on December 1, 2023")).toBe(
			"December 1, 2023",
		);
...
describe("URL Parsing", () => {
	test("Extension IDs can be extracted from URLs", () => {
		...
		).toBe("gighmmpiobklfepjocnamgkkbiglidom");
		expect(
			extractExtensionId(
				"https://chromewebstore.google.com/detail/adblock-plus/cfhdojbkjhnklbpkdaibdccddilifddb",
			),
		).toBe("cfhdojbkjhnklbpkdaibdccddilifddb");
		expect(
			extractExtensionId(
